...
_REDEFINES = re.compile(r"(?i)\bREDEFINES\s+([A-Z0-9][A-Z0-9-]*)")
_OCCURS = re.compile(r"(?i)\bOCCURS\s+(\d+)(?:\s+TO\s+(\d+))?")
_DEPENDING = re.compile(r"(?i)\bDEPENDING\s+ON\s+([A-Z0-9][A-Z0-9-]*)")
_USAGE = re.compile(r"(?i)\b(?:USAGE\s+(?:IS\s+)?)?(COMP-3|COMP-4|COMP-5|COMP|BINARY|PACKED-DECIMAL|DISPLAY)")
_VALUE = re.compile(r"(?i)\bVALUE\s+(?:IS\s+)?(.+?)(?:\.|$)")
...
def parse(text: str) -> Unit:
    ...
    for ln, txt in lines:
        ...
        if di:
            ...
            redefines = _REDEFINES.search(tail)
            occurs = _OCCURS.search(tail)
            depending = _DEPENDING.search(tail)
            usage = _USAGE.search(tail)
            value = _VALUE.search(tail)
            level = int(di.group(1))
            u.data_items.append({
                "level": level,
                "name": di.group(2).upper(),
...
def data_layout(items: list[dict]) -> list[dict]:
    ...
    for d in items:
        ...
        groups[sec].append({
            ...
            "occurs": d.get("occurs"),
            "occurs_to": d.get("occurs_to"),
            "depending_on": d.get("depending_on"),
            "usage": d.get("usage"),
            "value": d.get("value"),
