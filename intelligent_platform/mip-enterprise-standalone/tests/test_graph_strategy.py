...
class GraphStrategyTests(unittest.TestCase):
    ...
    def test_graph_slice_accepts_any_program_name_and_direction(self) -> None:
        downstream = self.api.graph_slice("CRDPOST", self.run_id, direction="downstream", depth=1, limit=50)
        upstream = self.api.graph_slice("CRDPOST", self.run_id, direction="upstream", depth=1, limit=50)

        self.assertEqual(downstream["direction"], "downstream")
        self.assertEqual(upstream["direction"], "upstream")
        self.assertTrue(downstream["nodes"])
        self.assertGreater(len(downstream["edges"]), len(upstream["edges"]))

    def test_node_catalog_supports_root_and_normal_program_filters(self) -> None:
        roots = self.api.nodes(self.run_id, scope="roots")["nodes"]
        programs = self.api.nodes(self.run_id, scope="programs")["nodes"]

        self.assertTrue(roots)
        self.assertTrue(programs)
        self.assertTrue(all(node["asset_type"] == "PROGRAM" for node in roots))
        self.assertGreaterEqual(len(programs), len(roots))
