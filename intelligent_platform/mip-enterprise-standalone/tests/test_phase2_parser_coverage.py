...
""",
    ...
    def test_db2_ddl_ignores_line_and_block_comments_in_columns(self) -> None:
        self.write(
            "sources/sql/DB2_DDL/CARDTXN",
            """
CREATE TABLE CARD_TXN
(
    CARD_NO CHAR(16) NOT NULL, -- COMMENTED_COL CHAR(1)
    /* BLOCKED_COL CHAR(8),
       BLOCKED_AMT DECIMAL(9,2), */
    AUTH_AMT DECIMAL(9,2) NOT NULL,
    PRIMARY KEY (CARD_NO)
);
""",
        )

        scan_mainframe_tree(self.root, self.db, run_id="db2-comments")

        attrs = json.loads(
            self.rows(
                """
                SELECT r.attributes_json
                FROM relationship r
                JOIN asset t ON t.asset_id = r.target_asset_id
                WHERE r.run_id = ? AND r.relationship_type = 'DEFINES_TABLE'
                  AND t.technical_name = 'CARD_TXN'
                """,
                ("db2-comments",),
            )[0]["attributes_json"]
        )
        columns = {column["name"] for column in attrs["columns"]}
        self.assertEqual(columns, {"CARD_NO", "AUTH_AMT"})
        self.assertEqual(attrs["primary_key"], ["CARD_NO"])
