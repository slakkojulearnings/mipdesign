...
def _strip_comment_paragraphs(text: str) -> str:
    """Drop IDENTIFICATION DIVISION comment-only paragraphs (AUTHOR. free text. …). The
    ProLeap main grammar models these as `commentEntry` consumed by the preprocessor; we
    simply remove the whole `KEYWORD. … .` entry so the grammar reaches PROGRAM-ID/DATA."""
    out = []
    skip = False
    for line in text.splitlines():
        head = line.strip().upper()
        if any(head.startswith(p + ".") or head == p for p in _COMMENT_PARAGRAPHS):
            skip = True
        if skip:
            if line.rstrip().endswith("."):             # entry ends at the period
                skip = False
            continue
        out.append(line)
    return "\n".join(out)
...
def to_unit(tree, text: str, pre: str | None = None) -> Unit:
    ...
    for d in _find_all(tree, "DataDescriptionEntryFormat1Context"):
        ...
        pm = cobol_ast._PIC.search(_source_slice(d, pre))
        pic = pm.group(1) if pm else None
        u.data_items.append({"level": int(level_m.group(1)),
                             "name": _txt(names[0]).strip(),
                             "pic": pic, "line": _line(d)})
