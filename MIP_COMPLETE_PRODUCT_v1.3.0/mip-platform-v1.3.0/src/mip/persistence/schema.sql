...
CREATE TABLE IF NOT EXISTS derived_insight (
    ...
    FOREIGN KEY(tenant_id) REFERENCES tenant(id),
    FOREIGN KEY(scan_id) REFERENCES scan_run(id) ON DELETE CASCADE,
    UNIQUE(scan_id, insight_type, subject, payload_json)
);

CREATE INDEX IF NOT EXISTS idx_insight_scan_type
ON derived_insight(scan_id, insight_type);

CREATE INDEX IF NOT EXISTS idx_insight_subject
ON derived_insight(scan_id, subject);
